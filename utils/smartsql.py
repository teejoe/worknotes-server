...
def quote(name):
    return None if name is None else "`%s`" % name.replace("`", "``")
...
class Field(object):
    ...
    def __eq__(self, f):
        ...
        if isinstance(f, collections.Iterable) and not isinstance(f, (str, unicode)):
            if len(f) < 1:
...
class UnionPart(object):
    ...
    def select(self, db=None):
        if db is not None:
            return db.select(self.sql, self.params, dict_cursor=True)

        if self.db is not None:
            return self.db.select(self.sql, self.params, dict_cursor=True)
...
QS, T, F, E, C = QuerySet, Table, Field, Expr, Condition
